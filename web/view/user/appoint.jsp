...
<table class="table table-bordered table-hover">
    <tr>
        <form action="user/SearchByKeywordsServlet" method="post">
    ...
    <tr>
        ...
        <td><a href="user/ChatToTeacherServlet?sno=<%=sno%>&teacherId=<%=u.getId()%>">进入聊天室</a>
        </td>
        <td>
            <form action="user/AppointServlet?id=<%=u.getId()%>&appointTime=<%=u.getAppointTime()%>&place=<%=u.getPlace()%>"
...
<nav aria-label="Page navigation">
    <ul class="pagination">
        ...
        <li>
            <a href="user/FindReleaseByUserServlet?currentPage=<%=pb.getCurrentPage()-1%>&rows=5" aria-label="Previous">
        ...
        %>
        <li class="active"><a href="user/FindReleaseByUserServlet?currentPage=<%=i%>&rows=5"><%=i%>
        </a></li>
        <%
        } else {
        %>
        <li><a href="user/FindReleaseByUserServlet?currentPage=<%=i%>&rows=5"><%=i%>
        </a></li>
        <%
                }
            }
        ...
        <li>
            <a href="user/FindReleaseByUserServlet?currentPage=<%=pb.getCurrentPage()+1%>&rows=5" aria-label="Next">
